    ...
    public void collidedWith (Participant p)
    {
        if (p instanceof AsteroidDestroyer)
        {
            if (size == 0)
            {
                ...
                controller.asteroidDestroyed(0);
            }
            else if (size == 1)
            {
                controller.makeSmallAsteroids(this.getX(), this.getY());
                controller.addScore(ASTEROID_SCORE[1]);
                
                Participant.expire(this);
                controller.asteroidDestroyed(1);
            }
            else
            {
                controller.makeMediumAsteroids(this.getX(), this.getY());
                controller.addScore(ASTEROID_SCORE[2]);
                
                Participant.expire(this);
                // Inform the controller
                controller.asteroidDestroyed(2);
            }
            controller.makeDebris(0, this.getX(), this.getY());
        }
    }
}
